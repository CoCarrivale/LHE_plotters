...
lhe_file_paths = {'EFT': 'lhe/full.lhe', 'SM': 'lhe/sm.lhe'}
    ...
    with open(lhe_file_path, 'r') as lhe_file:
        ...
        for line in lhe_file:
            if '<init>' in line:
                in_init_block = True
                continue
            if '</init>' in line or '<generator' in line:
                break
            if in_init_block:
                init_lines.append(line.strip())
        
        if len(init_lines) >= 2:
            cross_section = sum(float(line.split()[0]) for line in init_lines[1:])
...
ele_neutrino_pid = [12]  # e-, e+
mu_neutrino_pid = [14]      # mu-, mu+
        ...
        if elem.tag == 'event':
            ...
            if label not in data:
                data[label] = {'default': {
                    'invMass_W': [], ## mod
                    'invMass_Z': [], ## mod
                    'Lepton1_Pt': [], 'Lepton2_Pt': [], 'Lepton3_Pt': [], ## mod
                    'Lepton1_Eta': [], 'Lepton2_Eta': [], 'Lepton3_Eta': [], ## mod
                    'Jet1_Pt': [], 'Jet2_Pt': [],
                    'Jet_DeltaEta': []
                }}
            ...
            if len(leptons) >= 3:
                ...
                for idx, lepton in enumerate(leptons[:3]):  
                    ...
                    data[label]['default'][lepton_label_pt].append((pt, event_weight))
                    data[label]['default'][lepton_label_eta].append((eta, event_weight))
                ...
                if len(jets) >= 2:
                    ...
                    data[label]['default']['Jet1_Pt'].append((jet1_pt, event_weight))
                    data[label]['default']['Jet2_Pt'].append((jet2_pt, event_weight))
                    data[label]['default']['Jet_DeltaEta'].append((delta_eta, event_weight))

                if len(leptons) >= 3: ## mod
                    ...
                    if len(sfos_pairs) >= 1:
                        ...
                        remaining_indices = set(range(len(leptons))).difference(idx_Z)
                        Z_mass = None
                        W_mass = None

                        l_W = leptons[list(remaining_indices)[0]]  
                        neutrinos = [p for p in event_particles if p.id in ele_neutrino_pid + mu_neutrino_pid]

                        if len(neutrinos) == 1:
                            nu = neutrinos[0]
                            if (l_W.id in electron_pids and nu.id in ele_neutrino_pid) or (l_W.id in muon_pids and nu.id in mu_neutrino_pid):
                                W_mass = get_invariant_mass_two(l_W, nu)
                                Z_mass = float(best_pair[1])
                            else:
                                W_mass = 0.0
                                Z_mass = 0.0
                        else:
                        ...
                        data[label]['default']['invMass_Z'].append((Z_mass, event_weight))
                        data[label]['default']['invMass_W'].append((W_mass, event_weight))
...
for label, hist_data in data.items():
    ...
    print(f"  Events with less than 3 leptons: {less_lep[label]}")
...
plot_separated_histograms_with_error_lines(
    data=data,
    variables=['invMass_Z', 'invMass_W'],
    x_labels=['$M_{Z}$ [GeV]', '$M_{W}$ [GeV]'],
    x_ranges=[(70, 110), (60, 120)],
    bins=[6, 8],
    colors=colors,
    output_filename=os.path.join(output_folder, 'wz_mass.png'),
    scaling_factors=scaling_factors
)

plot_separated_histograms_with_error_lines(
    data=data,
    variables=['Lepton1_Pt', 'Lepton2_Pt', 'Lepton3_Pt'],
    x_labels=['$p_T (\ell_1)$ [GeV]', '$p_T (\ell_2)$ [GeV]', '$p_T (\ell_3)$ [GeV]'],
    x_ranges=[(0, 600), (0, 400), (0, 300)],
    bins=[10, 10, 10],
    colors=colors,
...
plot_separated_histograms_with_error_lines(
    data=data,
    variables=['Lepton1_Eta', 'Lepton2_Eta', 'Lepton3_Eta'],
    x_labels=['$\eta (\ell_1)$', '$\eta (\ell_2)$', '$\eta (\ell_3)$'],
    x_ranges=[(-5, 5), (-5, 5), (-5, 5)],
    bins=[10, 10, 10],
    colors=colors,
